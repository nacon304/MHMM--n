...
bool check_signed_ElGamal(vector<ll> &g, vector<ll> &m, vector<ll> &y, vector<ll> &r, vector<ll> &h, vector<ll> &mod)
{
    vector<ll> gm = powerModLongLong(g, m, mod);

    vector<ll> yr = powerModLongLong(y, r, mod);
    vector<ll> rh = powerModLongLong(r, h, mod);
    vector<ll> sgn = mulModLongLong(yr, rh, mod);
...
int main(int argc, char *argv[])
{
    ...
    inp >> y;
    string yn = hexToBin(y);
    vector<ll> yv = binToLongLong(yn);

    inp >> m;
    string mn = hexToBin(m);
    vector<ll> mv = binToLongLong(mn);

    inp >> r;
    string rn = hexToBin(r);
    vector<ll> rv = binToLongLong(rn);

    inp >> h;
    string hn = hexToBin(h);
    vector<ll> hv = binToLongLong(hn);

    out << check_signed_ElGamal(gv, mv, yv, rv, hv, pv);
